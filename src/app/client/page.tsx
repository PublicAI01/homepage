  ...
  return (
    <div className="mx-auto w-full max-w-[1120px] px-6">
      ...
      <header className="grid grid-cols-1 items-center gap-12 pt-10 pb-16 lg:grid-cols-[1.05fr_0.95fr] lg:pt-16 lg:pb-20">
        <div>
          <p className="text-p1 mb-4 text-xs tracking-[0.14em] uppercase">
          ...
          <h1 className="mb-5 text-4xl leading-[1.06] font-bold tracking-tight text-white md:text-5xl lg:text-[52px]">
            ...
            <br />
            <span className="text-p1">Not staged tasks.</span>
          </h1>
        ...
        <div
          ...
          <div className="flex items-center gap-2 bg-[#1E1E22] px-4 py-2.5 text-[11px] text-[#78758A]">
          ...
          <div className="min-h-[300px] p-[18px] text-[12.5px] leading-[1.9]">
            {specimen.map((ev, i) => (
              <div
                key={i}
                className="traj-ev">
                <span className="text-[#78758A]">{ev.t}</span>{' '}
                <span className="text-p1">{ev.k}</span> {ev.body}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="pb-20 lg:pb-24">
        ...
        <p className="text-g2 mb-8 max-w-[60ch]">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {inside.map(({ tag, text }) => (
            <div
              ...
              <span className="text-p1 mb-2 block text-[11px]">{tag}</span>
      ...
      <section className="pb-20 lg:pb-24">
        ...
        <div className="grid grid-cols-1 gap-3.5 lg:grid-cols-2">
          <div className="rounded-2xl border border-[#2C2C31] bg-white/[0.03] p-6">
            <h3 className="text-g2 mb-4 text-[15px] font-medium tracking-[0.06em] uppercase">
              Staged task data
            </h3>
            <ul className="list-none">
              {them.map((li) => (
                <li
                  key={li}
                  className="text-g2 border-t border-white/[0.06] py-[7px] text-sm">
                  {li}
                </li>
              ))}
            </ul>
          </div>
          <div className="border-primary bg-primary/10 rounded-2xl border p-6">
            <h3 className="text-p1 mb-4 text-[15px] font-medium tracking-[0.06em] uppercase">
            ...
            <ul className="list-none">
              {us.map((li) => (
                <li
                  key={li}
                  className="before:text-p1 border-t border-white/[0.06] py-[7px] text-sm text-[#D9D7E0] before:mr-1 before:content-['→']">
                  {li}
                </li>
              ))}
            </ul>
        ...
        <div className="flex items-stretch overflow-x-auto">
          {pipeline.map(({ tag, title, text }) => (
            <div
              ...
              <span className="mb-1.5 block text-[10.5px] tracking-[0.08em] text-[#78758A]">
                {tag}
              </span>
              <b className="mb-1 block text-[13.5px] font-medium text-white">
                {title}
              </b>
              <p className="text-g2 text-xs">{text}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          {rights.map(({ title, text }) => (
            <div
              ...
              <div>
                ...
                <p className="text-g2 text-[13px]">{text}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ===================== CTA ===================== */}
      <div className="border-primary from-primary/20 to-primary/[0.06] mb-20 flex flex-col items-center rounded-[20px] border bg-linear-135 p-11 text-center">
