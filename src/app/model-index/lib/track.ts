...
/** What a track is built from: its snapshot, its Overall weighting, its address. */
export interface TrackInput {
  data: IndexData;
  /** The boards that form this track's Overall, with their shares. */
  weighting: Weighting[];
  /** The track's page, e.g. https://publicai.io/model-index/image. */
  url: string;
  /** Where the API and MCP endpoints live; the text track's serve every track. */
  apiBase?: string;
...
/**
 * One read model for everything that answers questions about one track of
 * the index — the page, the API, the MCP server, the badge and the model
 * pages — built once per track from its snapshot. The text track is the
 * original; image and video are the same machinery over their own data.
 */
export function createTrack(input: TrackInput) {
  const { benchmarks, catalogs, excluded, generatedAt, models, scores } =
    input.data;
  const WEIGHTING = input.weighting;
  const OVERALL = new Set(WEIGHTING.map((w) => w.benchmarkId));
  const currentId = input.currentId ?? ((id: string) => id);
  const INDEX_URL = input.url;
  const API_BASE = input.apiBase ?? input.url;
  ...
  function scopeTotal(scope: Scope, includeReports = true) {
   ...
   */
  function positionIn(modelId: string, q: RankQuery = {}) {
  ...
  function rankModels(q: RankQuery = {}) {
    const listed = listScope(q);
   ...
   */
  function isHeadlineDomain(boards: number, models: number) {
    return (
  ...
  function headlineTaxonomy(): [string, string[]][] {
  ...
  function describeIndex() {
    ...
    return {
      ...
      method: [
        ...
        'A model with no Overall index gets an estimate ✱ (estimatedIndex): its figures on each shared capability measure are placed among models that have an index, and theirs is read at that position; outside their range the nearest anchor is a bound (ceiling or floor), not a point. Safety measures never anchor an estimate. Never a rank.',
      ],
