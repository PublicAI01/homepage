...
/**
 * One model's page, for any track: the text index at /model-index/m/<id>,
 * the image and video indexes under their own paths. The route files are
 * one line each; everything that reads the track is here.
 */
...
const load = async (track: TrackInfo, params: Promise<{ id: string }>) => {
  const { id } = await params;
  const found = track.index.modelStandings(decodeURIComponent(id));
...
export async function modelMetadata(
  ...
  const INDEX_URL = track.index.INDEX_URL;
  const found = await load(track, params);
  if (!found) return { title: `Model not found — ${track.title}` };
  const { model, standings } = found;
  const best = standings.find((s) => s.position !== null);
  const title = `${model.name} — ${track.title}`;
  const description = best
    ? `${model.name} (${model.org}) across ${standings.length} domains on the ${track.title}: #${best.position} of ${best.total} in ${best.scope}. Every figure traceable to its publisher.`
    : `${model.name} (${model.org}) on the ${track.title}.`;
  return {
    title,
    description,
...
export default async function ModelPage({
  ...
  const rankedTotal = track.index.describeIndex().counts.ranked;
    ...
    <div className="relative isolate before:absolute before:inset-y-0 before:left-1/2 before:-z-10 before:w-screen before:-translate-x-1/2 before:bg-[#08080A]">
      <div className="container mx-auto max-md:w-[calc(100vw-calc(var(--spacing-mobile-padding-x)*2))]">
        ...
        {/* For whoever publishes this model: the badge reads live, so a
            README pasted once stays current. The badge route reads the text
            track; the others get theirs when it learns to read them. */}
        {track.id === 'text' ? (
          <section className="pb-10">
