...
规模闸只数人要读的行:自动生成的回归快照(每个仓在 `SIZE_EXEMPT_RE` 里点名)不计入,
否则一个五行的解析修复会因为快照重跑两千行而被判「改动太大」转人工。
...
**一天最多一封。** `daily.sh` 把每个仓的运行结果攒进当天的 digest,最后统一发;
`run.sh` 单独跑时才自己发。修复会话不发信(它只负责把「跳过项·大白话总结」写在输出末尾,
脚本原样摘走)。信里会有:

- 有 P0/P1 被**跳过**(信心 <95% / 属判决域)—— 附大白话总结。**同一条只说一次**:每条
  跳过项带一个稳定代号 `[skip:…]`,记在 `<state>/skips.json`;已经说过的不再重复,
  除非过了 14 天还没解决,才提醒一次。还开着的清单会喂给第二天的修复会话,让它认得出来。
- 任何一道闸拦下了整批 —— 附修复方自述和跳过项
- 扫描/修复/复审跑挂了或无产物
- **一切正常、修好推上去了、没有跳过项 → 不发信**。安静就是没事。

GitHub Actions 自己的 "Run failed" 邮件是另一回事,那是 GitHub 账号级通知,和这里无关;
嫌重复可在 GitHub → Settings → Notifications → Actions 关掉,`refresh.yml` 失败时
我们自己的告警信内容更具体。
